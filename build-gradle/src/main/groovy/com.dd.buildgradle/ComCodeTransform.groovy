...
//                        System.out.println("classNameTemp " + classNameTemp);
...
//            autoLoadComCode.append(ctClass.getName() + "()" + ".onCreate();")
        }
        String autoCode = autoLoadComCode.toString();
        return autoCode;
    }
    ...
    private boolean isActivator(CtClass ctClass) {
        try {
            for (CtClass ctClassInter : ctClass.getInterfaces()) {
                if ("com.nhzw.component.lib.applicationlike.IApplicationLike".equals(ctClassInter.name)) {
                    return true;
                }
            }
        } catch (Exception e) {
